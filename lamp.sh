...
sudo mysql -uroot -e "GRANT ALL PRIVILEGES ON Victor.* TO 'Ayo'@'localhost';"
cd /var/www/laravel
...
sudo sed -i '25 s/=laravel/=Victor/' /var/www/laravel/.env
sudo sed -i '26 s/=root/=Ayo/' /var/www/laravel/.env
sudo sed -i '27 s/=/=Joseph/' /var/www/laravel/.env
